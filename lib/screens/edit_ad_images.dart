...
class _EditAdImagesPageState extends State<EditAdImagesPage> {
  ...
  void initState() {
    super.initState();
    AnalyticsEngine().logScreenViewed(screenName: 'edit_ad_images');
    if (widget.editingAdData['image_urls'] != null) {
      _images.addAll(List<String>.from(widget.editingAdData['image_urls']));
    }
  }
      ...
      if (picked.isNotEmpty) {
        ...
        if (hasLargeImages && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content:
                  Text('تم استبعاد بعض الصور لأن حجمها يتجاوز 5 ميجابايت.'),
      ...
      if (quality == 'blurry' || quality == 'dark') {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  'هذه الصورة تبدو ${quality == 'blurry' ? 'غير واضحة' : 'مظلمة'}. الصور الواضحة تباع أسرع بـ 3 أضعاف!'),
  ...
  Future<void> _nextStep() async {
    if (_images.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('الرجاء اختيار صور للإعلان'),
            backgroundColor: Colors.red),
      );
      return;
    }
    ...
    for (int i = 0; i < _images.length; i++) {
      if (_images[i] is String) {
        uploadedUrls.add(_images[i]);
        continue;
      }

      final XFile file = _images[i];
      if (_uploadedPathsCache.containsKey(file.path)) {
        uploadedUrls.add(_uploadedPathsCache[file.path]!);
      ...
      try {
        final url = await ApiService().uploadSingleMedia(file);
        _uploadedPathsCache[file.path] = url; // Cache the successful upload
    ...
    if (uploadedUrls.isEmpty || uploadedUrls.length < 3) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('لابد من رفع 3 صور صالحة على الأقل.'),
            backgroundColor: Colors.red),
      );
      return;
    }
    ...
    AnalyticsEngine()
        .logButtonTapped(buttonName: 'next_step', location: 'edit_ad_images');
    widget.editingAdData['image_urls'] = uploadedUrls;
    if (uploadedUrls.isNotEmpty) {
      widget.editingAdData['image_url'] = uploadedUrls.first;
    }
  ...
  void _showRejectedImagesDialog(List<Map<String, dynamic>> rejectedImages) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('عذراً، تم رفض بعض الصور',
            style: TextStyle(
        ...
        actions: [
          TextButton(
            onPressed: () {
              ...
              setState(() {
                final toRemove =
                    rejectedImages.map((e) => e['file'] as XFile).toSet();
    ...
    return Stack(
      children: [
        Scaffold(
          ...
          appBar: AppBar(
            title: const Text('تعديل صور الإعلان',
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'اختر أفضل صورة لتكون الغلاف الأساسي لإعلانك لجذب المزيد من المشترين.',
                        style: TextStyle(
                            fontSize: 15,
                          ...
                          children: [
                            const Padding(
                              ...
                              child: Icon(Icons.shield_outlined,
                                  color: Color(0xFFE94057), size: 18),
                            ),
                    ...
                    children: [
                      ...
                      if (_images.isEmpty)
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton.icon(
                              ...
                              icon: const Icon(
                                  Icons.add_photo_alternate_rounded,
                                  size: 20),
                              label: const Text('تعديل صور الإعلان',
                                  style: TextStyle(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 16)),
                        ...
                        if (_images.isNotEmpty) ...[
                          const Text('الصورة الرئيسية (الغلاف)',
                              style: TextStyle(
                              ...
                              child: Row(
                                children: [
                                  const Icon(Icons.auto_awesome,
                                      color: Color(0xFFE94057), size: 16),
                                  const SizedBox(width: 6),
                                  Text('تصنيف مقترح: $_suggestedCategory',
                              ...
                              RichText(
                                text: TextSpan(
                                  style: const TextStyle(
                                      fontFamily: 'Cairo',
                                      color: Colors
                                          .black87), // Assuming Cairo or similar is default
                                  children: [
                                    const TextSpan(
                                        text: 'صور إضافية ',
                                        style: TextStyle(
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      const Icon(Icons.info_outline_rounded,
                                          color: Color(0xFF0075FF), size: 22),
                                      const SizedBox(width: 8),
                                      Text(
                                  ...
                                  SizedBox(
                                    ...
                                    child: ElevatedButton.icon(
                                      ...
                                      style: ElevatedButton.styleFrom(
                                        ...
                                        shadowColor: const Color(0xFF0075FF)
                                            .withOpacity(0.4),
                                      ).copyWith(
                                        elevation: WidgetStateProperty
                                            .resolveWith<double>((states) {
                                          if (states.contains(
                                              WidgetState.pressed)) return 0;
                                          return 6;
                                        }),
                                      ),
                                    ),
            ...
            child: SafeArea(
              child: SizedBox(
                ...
                child: ElevatedButton(
                  onPressed:
                      (_images.length >= 3 && !_isUploading) ? _nextStep : null,
                  style: ElevatedButton.styleFrom(
                    elevation: 0,
                    padding: const EdgeInsets.symmetric(vertical: 18),
                    shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(16)),
                    backgroundColor: _images.length >= 3
                        ? const Color(0xFF0075FF)
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(
                            color:
                                (isError ? Colors.red : const Color(0xFF0075FF))
                                    .withOpacity(0.15),
                            blurRadius: 40,
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            isError
                                ? 'خطأ في الصور'
                                : 'جاري فحص الصور ($_analyzedCount/$_totalToAnalyze)',
                            style: TextStyle(
                              fontSize: 22,
                              fontWeight: FontWeight.w900,
                              color: isError
  ...
  Widget _buildFeaturedImageCard() {
    bool isFailed = _images.isNotEmpty &&
        _images[0] is XFile &&
        _failedImages.contains(_images[0].path);
    bool isSuccess = _images.isNotEmpty &&
        _images[0] is XFile &&
        _successImages.contains(_images[0].path);
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            image: DecorationImage(
              image: _images[0] is String
                  ? NetworkImage(_images[0].startsWith('http')
                      ? _images[0]
                      : 'https://api.sooqcom.app' + _images[0]) as ImageProvider
                  : FileImage(File((_images[0] as XFile).path)),
              fit: BoxFit.cover,
            ),
          ),
        ),
  ...
  Widget _buildThumbnailItem(int index) {
    bool isFailed =
        _images[index] is XFile && _failedImages.contains(_images[index].path);
    bool isSuccess =
        _images[index] is XFile && _successImages.contains(_images[index].path);
          ...
          decoration: BoxDecoration(
            ...
            image: DecorationImage(
              image: _images[index] is String
                  ? NetworkImage(_images[index].startsWith('http')
                          ? _images[index]
                          : 'https://api.sooqcom.app' + _images[index])
                      as ImageProvider
                  : FileImage(File((_images[index] as XFile).path)),
              fit: BoxFit.cover,
            ),
          ),
        ),
