...
class _PremiumRealEstateCardState extends State<PremiumRealEstateCard>
  ...
  void initState() {
    ...
    _negotiable = widget.ad.tags
            .any((t) => t.contains('تفاوض') || t.contains('negotiable')) ||
        lowerDesc.contains('قابل للتفاوض') ||
        lowerDesc.contains('تفاوض');

    _mortgage = widget.ad.tags.any((t) =>
            t.contains('تقسيط') ||
            t.contains('اقساط') ||
            t.contains('installment')) ||
        lowerDesc.contains('تقسيط') ||
        lowerDesc.contains('اقساط');
    _has360 = widget.ad.attributes != null &&
        widget.ad.attributes!['virtual_tour_url'] != null;
    _hasVideo = widget.ad.attributes != null &&
        widget.ad.attributes!['video_url'] != null;
  ...
  Future<void> _toggleFavorite() async {
    ...
    final currentUserId =
        context.read<AuthProvider>().userData?['sub']?.toString();
    ...
    try {
      ...
      if (mounted) {
        ...
        context
            .read<AppProvider>()
            .toggleFavoriteCount(isNowSaved, adId: widget.ad.id);
      }
    } catch (e) {
      if (mounted) {
        ...
        if (e.toString().contains('Unauthorized')) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: const Text('حدث خطأ أثناء حفظ الإعلان.',
                style: TextStyle(fontWeight: FontWeight.w600)),
            behavior: SnackBarBehavior.floating,
            backgroundColor: const Color(0xFF2D2D2D),
            shape:
                ...
                itemBuilder: (context, index) {
                  return Stack(
                    ...
                    children: [
                      _hasVideo && index == 0
                          ? PremiumVideoPlayer(
                              videoUrl: widget.ad.attributes!['video_url'] ??
                                  widget.ad.videoUrl!,
              ...
              Positioned(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color:
                          Colors.black.withOpacity(0.65), // Darker background
                      shape: BoxShape.circle, // Circular shape looks cleaner
                    ),
                    child: const Icon(Icons.fullscreen_rounded,
                        color: Colors.white, size: 26), // Bigger icon
                  ),
                ),
              ),
            ],
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 4),
              Text(
                  widget.ad.createdAt != null
                      ? _formatTimeAgo(widget.ad.createdAt!)
                      : 'قبل ساعتين',
                  style: TextStyle(
              ...
              if (widget.ad.views >= 10) ...[
                ...
                Icon(Icons.remove_red_eye_outlined,
                    size: 12, color: Colors.blueGrey.shade400),
                const SizedBox(width: 4),
                Text(_formatNumber(widget.ad.views),
                    style: TextStyle(
                        color: Colors.blueGrey.shade400,
                        fontSize: 11,
                        fontWeight: FontWeight.w500)),
              ...
              if (widget.ad.favoritesCount >= 10) ...[
                ...
                Icon(Icons.favorite_border,
                    size: 12, color: Colors.blueGrey.shade400),
                const SizedBox(width: 4),
                Text(_formatNumber(widget.ad.favoritesCount),
                    style: TextStyle(
                        color: Colors.blueGrey.shade400,
                        fontSize: 11,
                        fontWeight: FontWeight.w500)),
  ...
  Widget _buildHighlightedTitle() {
    ...
    final queryWords = widget.searchQuery!
        .toLowerCase()
        .split(' ')
        .where((w) => w.length > 1)
        .toList();
    if (queryWords.isEmpty) {
      return Text(widget.ad.title,
          maxLines: 2,
          overflow: TextOverflow.ellipsis,
          style: const TextStyle(
              fontWeight: FontWeight.w900,
    ...
    for (RegExpMatch match in matches) {
      if (match.start > currentIndex) {
        spans.add(TextSpan(
            text: widget.ad.title.substring(currentIndex, match.start)));
      }
      spans.add(TextSpan(
        text: match.group(0),
        style: const TextStyle(
            color: Color(0xFF0075FF), backgroundColor: Color(0xFFEFF6FF)),
      ));
  ...
  Widget _buildSpecsRow() {
    ...
    if (widget.ad.sharedRoomDetails != null) {
      ...
      if (dynFurnished == 'مفروش' ||
          dynFurnished == 'Yes' ||
          dynFurnished == 'نعم') {
        furnishedStatus = 'مفروشة';
      } else if (dynFurnished == 'غير مفروش' ||
          dynFurnished == 'No' ||
          dynFurnished == 'لا') {
    ...
    if (widget.ad.attributes != null) {
      final dyn =
          widget.ad.attributes!['dynamic_data'] as Map<String, dynamic>? ?? {};

      var areaRaw = widget.ad.attributes!['area'] ?? dyn['area'];
      if (areaRaw != null) {
        area =
            int.tryParse(areaRaw.toString().replaceAll(RegExp(r'[^0-9]'), ''));
      }

      var bedsRaw = widget.ad.attributes!['rooms'] ?? dyn['bedrooms'];
      if (beds == null && bedsRaw != null) {
        beds =
            int.tryParse(bedsRaw.toString().replaceAll(RegExp(r'[^0-9]'), ''));
      }

      var bathsRaw = widget.ad.attributes!['bathrooms'] ?? dyn['bathrooms'];
      if (baths == null && bathsRaw != null) {
        baths =
            int.tryParse(bathsRaw.toString().replaceAll(RegExp(r'[^0-9]'), ''));
      }
      ...
      var rentRaw =
          widget.ad.attributes!['rent_duration'] ?? dyn['rent_duration'];
      if (rentDuration == null && rentRaw != null) {
        rentDuration = rentRaw.toString();
      }

      var zoningRaw = widget.ad.attributes!['zoning_classification'] ??
          dyn['zoning_classification'];
      if (zoningClassification == null && zoningRaw != null) {
        zoningClassification = zoningRaw.toString();
      }

      var facadeRaw =
          widget.ad.attributes!['street_facade'] ?? dyn['street_facade'];
      ...
      if (topography == null && topoRaw != null)
        topography = topoRaw.toString();

      var ownerRaw =
          widget.ad.attributes!['ownership_type'] ?? dyn['ownership_type'];
      if (ownershipType == null && ownerRaw != null)
        ownershipType = ownerRaw.toString();
    }
    ...
    if (beds != null && beds > 0)
      specChildren.add(_specItem(Icons.bed_outlined, '$beds غرف نوم'));
    if (baths != null && baths > 0)
      specChildren.add(_specItem(Icons.bathtub_outlined, '$baths حمّامات'));
    if (rentDuration != null && rentDuration!.isNotEmpty)
      specChildren.add(_specItem(Icons.calendar_month_outlined, rentDuration!));
    if (furnishedStatus != null && furnishedStatus!.isNotEmpty)
      specChildren.add(_specItem(Icons.chair_outlined, furnishedStatus!));
    if (area != null && area! > 0)
      specChildren.add(_specItem(Icons.square_foot, '$area م٢'));
    if (zoningClassification != null && zoningClassification!.isNotEmpty)
      specChildren
          .add(_specItem(Icons.category_outlined, zoningClassification!));
    if (streetFacade != null && streetFacade!.isNotEmpty)
      specChildren.add(_specItem(Icons.straighten_outlined, streetFacade!));
    if (topography != null && topography!.isNotEmpty)
      specChildren.add(_specItem(Icons.terrain_outlined, topography!));
    if (ownershipType != null && ownershipType!.isNotEmpty)
      specChildren
          .add(_specItem(Icons.assignment_ind_outlined, ownershipType!));
    if (floor != null && floor!.isNotEmpty) {
      final String floorDisplay =
          floor!.contains('طابق') || floor!.contains('الطابق')
              ? floor!
              : 'الطابق $floor';
  ...
  Widget _buildSmartHighlightsRow() {
    ...
    if (tags.isEmpty && widget.ad.sharedRoomDetails != null) {
      final dynFurnished = widget.ad.sharedRoomDetails!.furnished;
      if (dynFurnished != null && dynFurnished.isNotEmpty)
        tags.add('🛋️ $dynFurnished');
    }
            ...
            child: _actionButton(
                ...
                onTap: () async {
                  ...
                  if (phone == null || phone.isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content:
                            const Text('رقم الهاتف غير متوفر لهذه القائمة'),
                        behavior: SnackBarBehavior.floating,
